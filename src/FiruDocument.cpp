...
_LIT( KNameSuomi, "suomi.txt" );
_LIT( KNameRussian, "russian.txt" );
...
void CFiruDocument::ImportDictionaryL( )
{
    ...
    RFile suomi;
    User::LeaveIfError( suomi.Open( iFsSession, KNameSuomi( ), EFileShareReadersOnly
        | EFileStream ) );
    CleanupClosePushL( suomi );

    RFile russian;
    User::LeaveIfError( russian.Open( iFsSession, KNameRussian( ),
        EFileShareReadersOnly | EFileStream ) );
    CleanupClosePushL( russian );

    RBuf8 fi, ru; // any string should fit here, rest is ignored
    fi.CreateL( 64 );
    CleanupClosePushL( fi );
    ru.CreateL( 64 );
    CleanupClosePushL( ru );

    TInt batch = 0;
    TBool stop = EFalse;
    while ( !stop )
    {
        HBufC* fin = ReadLineLC( suomi, fi );
...
//        RDebug::Print( _L("rus: %S"), rus );
    ...
    CleanupStack::PopAndDestroy( &suomi );
...
HBufC* CFiruDocument::ReadLineLC( RFile& aFile, RBuf8& aBuffer )
{
    ...
    if ( pos >= 0 )
    {
        ...
        TInt err = CnvUtfConverter::ConvertToUnicodeFromUtf8( str,
            aBuffer.Left( pos ) );
        if ( err )
        {
