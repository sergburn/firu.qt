    ...
    )
{
    ...
    if ( flags & SQLITE_OPEN_CREATE )
    {
        ...
        {
            ret = pFile->file.Open( RFS( pVfs ), filename, EFileStream | EFileWrite );
        }
        else
        {
            ret = pFile->file.Create( RFS( pVfs ), filename, EFileStream | EFileWrite );
        }
    }
    ...
    {
        ret = pFile->file.Open( RFS( pVfs ), filename, EFileStream | EFileRead );
    }
...
)
{
    ...
    TPtr8 full( zFull, MAX_FILE_NAME );
